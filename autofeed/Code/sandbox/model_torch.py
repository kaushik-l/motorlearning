...
class Network:
    def __init__(self, name='Ctx', N=200, Nl = 100, M = 10):
        self.name = name
        # network parameters
        self.N = N  # RNN units
        ...
        elif self.name == 'CtxDual':
            self.Nl, self.Nu = Nl, N - Nl
            ws0 = self.g_in * np.random.standard_normal([self.S, self.N]).astype(np.float32) / np.sqrt(self.S)
            Juu0 = self.g_rec * np.random.standard_normal([self.Nu, self.Nu]).astype(np.float32) / np.sqrt(self.Nu)
            Jul0 = self.g_rec * np.zeros([self.Nl, self.Nu]).astype(np.float32) / np.sqrt(self.Nl)
            Jlu0 = self.g_rec * np.random.standard_normal([self.Nu, self.Nl]).astype(np.float32) / np.sqrt(self.Nu)
            Jll0 = self.g_rec * np.random.standard_normal([self.Nl, self.Nl]).astype(np.float32) / np.sqrt(self.Nl)
            wr0 = self.g_out * np.random.standard_normal([self.N, self.R]).astype(np.float32) / np.sqrt(self.Nl)
            b0 = np.zeros([1, self.N]).astype(np.float32)
            self.ws = torch.tensor(ws0, requires_grad=True)
            self.Juu = torch.tensor(Juu0, requires_grad=True)
            self.Jul = torch.tensor(Jul0, requires_grad=True)
            self.Jlu = torch.tensor(Jlu0, requires_grad=True)
            self.Jll = torch.tensor(Jll0, requires_grad=True)
            self.wr = torch.tensor(wr0, requires_grad=True)
            self.b = torch.tensor(b0, requires_grad=True)
            # mask for zero weights
            mask_ws = torch.cat([torch.ones(self.S, self.Nu).T, torch.zeros(self.S, self.Nl).T]).T
            mask_wr = torch.cat([torch.zeros(self.R, self.Nu).T, torch.ones(self.R, self.Nl).T])
            self.ws.data = self.ws.data * mask_ws
            self.wr.data = self.wr.data * mask_wr
        elif self.name == 'ThCtx':
            self.M = M
            ws0 = self.g_in * np.random.standard_normal([self.S, self.N]).astype(np.float32) / np.sqrt(self.S)
            J0 = self.g_rec * np.random.standard_normal([self.N, self.N]).astype(np.float32) / np.sqrt(self.N)
            wr0 = self.g_out * np.random.standard_normal([self.N, self.R]).astype(np.float32) / np.sqrt(self.N)
            b0 = np.zeros([1, self.N]).astype(np.float32)
            U0 = np.sqrt(self.g_rec) * np.random.standard_normal([self.N, self.M]).astype(np.float32) / np.sqrt(self.N)
            V0 = np.sqrt(self.g_rec) * np.random.standard_normal([self.M, self.N]).astype(np.float32) / np.sqrt(self.M)
            self.ws = torch.tensor(ws0, requires_grad=True)
            self.J = torch.tensor(J0, requires_grad=True)
            self.U = torch.tensor(U0, requires_grad=True)
            self.V = torch.tensor(V0, requires_grad=True)
            self.wr = torch.tensor(wr0, requires_grad=True)
            self.b = torch.tensor(b0, requires_grad=True)
        elif self.name == 'ThCtxDual':
            self.Nl, self.Nu, self.M = Nl, N - Nl, M
            wsu0 = self.g_in * np.random.standard_normal([self.S, self.Nu]).astype(np.float32) / np.sqrt(self.S)
            #wsl0 = self.g_in * np.zeros([self.S, self.Nl]).astype(np.float32) / np.sqrt(self.S)
            wsl0 = self.g_in * np.random.standard_normal([self.S, self.Nu]).astype(np.float32) / np.sqrt(self.S)
            wsl0[[2, 3, -2, -1], :] = 0
            Juu0 = self.g_rec * np.random.standard_normal([self.Nu, self.Nu]).astype(np.float32) / np.sqrt(self.Nu)
            Jul0 = self.g_rec * np.zeros([self.Nl, self.Nu]).astype(np.float32) / np.sqrt(self.Nl)
            Jlu0 = self.g_rec * np.random.standard_normal([self.Nu, self.Nl]).astype(np.float32) / np.sqrt(self.Nu)
            Jll0 = self.g_rec * np.random.standard_normal([self.Nl, self.Nl]).astype(np.float32) / np.sqrt(self.Nl)
            wru0 = self.g_out * np.zeros([self.Nu, self.R]).astype(np.float32) / np.sqrt(self.Nu)
            wrl0 = self.g_out * np.random.standard_normal([self.Nl, self.R]).astype(np.float32) / np.sqrt(self.Nl)
            b0 = np.zeros([1, self.N]).astype(np.float32)
            Ul0 = np.sqrt(self.g_rec) * np.zeros([self.Nl, self.M]).astype(np.float32) / np.sqrt(self.Nl)
            Uu0 = np.sqrt(self.g_rec) * np.zeros([self.Nu, self.M]).astype(np.float32) / np.sqrt(self.Nu)
            Vl0 = np.sqrt(self.g_rec) * np.zeros([self.M, self.Nl]).astype(np.float32) / np.sqrt(self.M)
            Vu0 = np.sqrt(self.g_rec) * np.zeros([self.M, self.Nu]).astype(np.float32) / np.sqrt(self.M)
            self.wsu = torch.tensor(wsu0, requires_grad=True)
            self.wsl = torch.tensor(wsl0, requires_grad=True)
            self.Juu = torch.tensor(Juu0, requires_grad=True)
            self.Jul = torch.tensor(Jul0, requires_grad=False)
            self.Jlu = torch.tensor(Jlu0, requires_grad=True)
            self.Jll = torch.tensor(Jll0, requires_grad=True)
            self.Ul = torch.tensor(Ul0, requires_grad=False)
            self.Uu = torch.tensor(Uu0, requires_grad=False)
            self.Vl = torch.tensor(Vl0, requires_grad=False)
            self.Vu = torch.tensor(Vu0, requires_grad=False)
            self.wru = torch.tensor(wru0, requires_grad=False)
            self.wrl = torch.tensor(wrl0, requires_grad=True)
            self.b = torch.tensor(b0, requires_grad=True)
...
class Task:
    def __init__(self, name='Reaching', rand_tar=True, num_tar=None, auto=False, feed=False, both=False):
        ...
        self.switch_after = .5  # fraction of epochs before multitasking -- n/a unless both=True && (auto=True or feed=True)
...
class Trial:
    ...
    def add_context(self, task, B=1):
        if task.feed and not task.auto:
            self.c = torch.as_tensor(np.tile(np.array(task.c_feed, dtype=np.float32), (B, 1)))
        elif task.auto and not task.feed:
            self.c = torch.as_tensor(np.tile(np.array(task.c_auto, dtype=np.float32), (B, 1)))
        else:  # randomly interleave auto and feed:
            c = torch.cat((torch.as_tensor(np.tile(np.array(task.c_feed, dtype=np.float32), (int(B / 2), 1))),
                           torch.as_tensor(np.tile(np.array(task.c_auto, dtype=np.float32), (int(B / 2), 1)))))
            self.c = c[torch.randperm(B)]
