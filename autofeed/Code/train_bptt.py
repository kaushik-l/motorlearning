...
ws0 = net.g_in * np.random.standard_normal([net.S, net.N]).astype(np.float32) / np.sqrt(net.S)
J0 = net.g_rec * np.random.standard_normal([net.N, net.N]).astype(np.float32) / np.sqrt(net.N)
wr0 = net.g_out * np.random.standard_normal([net.N, net.R]).astype(np.float32) / np.sqrt(net.N)
b0 = np.zeros([1, net.N]).astype(np.float32)
net.ws = torch.tensor(ws0, requires_grad=True)
net.J = torch.tensor(J0, requires_grad=True)
net.wr = torch.tensor(wr0, requires_grad=True)
net.b = torch.tensor(b0, requires_grad=True)
...
torch.save({'net': net, 'task': task, 'plant': plant, 'algo': algo, 'mses': mses, 'add_sensoryprediction': add_sensoryprediction,
           ...
           'C:\\Users\\jkl9\\PycharmProjects\\autofeed\\Data\\feed.pt')